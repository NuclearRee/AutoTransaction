  ...
  public class NumCalculation
    {
        ...
        public static string GetSaleNum(string  [] _positionlist, string [] _datalist, double[] _a_param, double[] _b_param, double[]  _c_param)
        {
            ...
            //盈亏比例 = (现价 - 成本价）/成本价
            var Ratio = (Convert.ToDouble(_datalist[3]) - Convert.ToDouble(_positionlist[2]))/ Convert.ToDouble(_positionlist[2]);
            if(Ratio > 0)
            {
                //当盈亏比例〉A1%且盈亏比例<A2%时，当天最高价回落B1%时卖出可用股份C1%股份
                if (Ratio > _a_param[0] && Ratio <= _a_param[1] && B >= _b_param[0])
                {
                    return Convert.ToInt32(_c_param[0] * Convert.ToDouble(_positionlist[1])).ToString();
                }
                //当盈亏比例〉A2%且盈亏比例<A3%时，当天最高价回落B2%时卖出可用股份C2%股份
                else if (Ratio > _a_param[1] && Ratio <= _a_param[2] && B >= _b_param[1])
                {
                    return Convert.ToInt32(_c_param[1] * Convert.ToDouble(_positionlist[1])).ToString();
                }
                //当盈亏比例〉A3%时，当天最高价回落B3%时卖出可用股份100%的全部股份
                else if (Ratio > _a_param[2] && B >= _b_param[2])
                {
                    return Convert.ToInt32(_positionlist[1]).ToString();
                }
                //其他情况返回0
                else
                {
            ...
            else
            {
                ...
                {
                    return Convert.ToInt32(_c_param[2]*Convert.ToDouble(_positionlist[1])).ToString();
                }
                //当盈亏比例<-A5%时，卖出可用股份C4%股份
                else if (Ratio < -_a_param[4])
                {
                    return Convert.ToInt32(_c_param[3] * Convert.ToDouble(_positionlist[1])).ToString();
                }
                //其他情况返回0
                else
                {
